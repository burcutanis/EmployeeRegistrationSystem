...
const UserPersonalInformation = require('../../models/User/UserPersonalInfo');
...
router.post('/',
    ...
    async (req, res) => {
        ...
        try {
            var newPersonalInformation = new UserPersonalInformation({
...
router.get('/', async (req, res) => {
    try {
        const personalInformations = await UserPersonalInformation.find().sort({ date: -1 });
...
router.delete('/:id', async (req, res) => {
    try {
        ...
        const personalInformation = await UserPersonalInformation.findOneAndRemove(query);
...
router.get('/:id', async (req, res) => {
    try {
        const personalInformation = await UserPersonalInformation.findById(req.params.id);
...
router.put('/:id', async (req, res) => {
    try {
        ...
        const personalInformation = await UserPersonalInformation.findByIdAndUpdate(id, req.body, { useFindAndModify: false });
...
router.get('/user/:id', async (req, res) => {
    try {
        var query = { userId: req.params.id };
        const personalInformation = await UserPersonalInformation.find(query);
        res.json(personalInformation);
        if (!personalInformation) {
            return res.status(404).json({ msg: 'personalInformation not found' });
        }
...
router.delete('/:userId/:status', async (req, res) => {
    try {
        ...
        res.json({ msg: 'personalInformation item removed' });
    } catch (err) {
        console.error(err.message);
