...
const ExperienceItem = require('../../models/ExperienceItem');
...
router.post('/',
    ...
    async (req, res) => {
        ...
        try {
            var newExperience = new ExperienceItem({
...
router.get('/', async (req, res) => {
    try {
        const experience = await ExperienceItem.find().sort({ date: -1 });
        res.json(experience);
    } catch (err) {
        console.error(err.message);
        res.status(500).send('Server Error');
...
router.delete('/:id', async (req, res) => {
    try {
        ...
        experience = await ExperienceItem.deleteMany(query);
...
router.delete('/:id2/:id', async (req, res) => {
    try {
        ...
        const experience = await ExperienceItem.findOneAndRemove(query);
...
router.get('/:id', async (req, res) => {
    try {
        const experience = await ExperienceItem.findById(req.params.id);
        res.json(experience);
        if (!experience) {
...
router.put('/:id', async (req, res) => {
    try {
        ...
        const experienceItem = await ExperienceItem.findByIdAndUpdate(id, req.body, { useFindAndModify: false });
...
router.get('/user/:id', async (req, res) => {
    try {
        var query = { userId: req.params.id };
        const experienceItem = await ExperienceItem.find(query);
